...
VIDEOS_DIR = os.path.join(PROJECT_ROOT, "videos")  # Project root videos directory
FINAL_LIBRARY = os.path.join(PROJECT_ROOT, "Strokes_Library")
...
def load_segments_from_llc(llc_path):
    ...
    Supports both:
    1) JSON format from LosslessCut (cutSegments array with start/end times)
    2) Simple text format (start_time end_time label per line)
    ...
    # Check if it looks like JSON (has curly braces and contains cutSegments)
    if "{" in content and "cutSegments" in content:
        try:
            # Try to parse as JSON or convert to valid JSON
            try:
                data = json.loads(content)
            except json.JSONDecodeError:
                valid_text = convert_llc_to_valid_json(content)
            ...
            print(f"[INFO] Loaded {len(segments)} segments from JSON LLC file: {llc_path}")
            return segments
            
        except Exception as e:
            print(f"[WARN] Failed to parse LLC file as JSON: {e}")
            # Fall through to try text format
    
    # Try to parse as text format (start_time end_time label)
    segments = []
    ...
    if segments:
        print(f"[INFO] Loaded {len(segments)} segments from text LLC file: {llc_path}")
    else:
        print(f"[WARN] No valid segments found in LLC file: {llc_path}")
